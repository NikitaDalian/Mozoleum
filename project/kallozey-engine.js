// КАЛЛОЗЕЙ — движок генерации мозолей.
// Архитектура: «одна геометрия — много стилей».
// Из seed детерминированно выводится «анатомия» (форма + параметры),
// поверх неё накладывается стиль рендера. Один seed = одна и та же мозоль.

// ---------- PRNG (mulberry32) + независимые потоки от seed ----------
...
function stream(seed, salt) { return mulberry32((seed ^ salt) >>> 0); }
export function rollSeed() { return (Math.random() * 0xffffffff) >>> 0; }
...
// Сертификация подлинности — отдельная гача на печати
export const CERTS = [
...
const RARITY_EPITHET = { legendary: 'Magnus', mythic: 'Aeternus' };
...
// ---------- Библиотека происхождений (ядро юмора) ----------
...
export const LOCATIONS = Array.from(new Set(ORIGINS.map((o) => o.loc)));

// Группировка происхождений по виду деятельности (для понятного списка)
...
export function originsByCategory() {
...
// ---------- Построение мозоли ----------
export function buildCallus({ originId, seed, stageKey = 'auto', locOverride = null }) {
  const origin = ORIGINS.find((o) => o.id === originId) || ORIGINS[0];
  const loc = locOverride || origin.loc;

  const rs = stream(seed, 0x9e3779b9); // форма
  const rr = stream(seed, 0x85ebca6b); // редкость
  const rt = stream(seed, 0xc2b2ae35); // тех. данные / имя

  // Стадия — всегда от seed (скрыта от пользователя)
  const stageRoll = rs();
  ...
  // Редкость (гача): чем выше стадия — тем выше шанс редкой (но топы остаются редкими)
  ...
  // Сертификация печати (своя гача, с бонусом от редкости)
  ...
  // Слои-контуры: больше при высокой стадии
  const layers = clamp(2 + Math.round(patina * 3) + (rs() < 0.45 ? 1 : 0), 2, 6);

  // Гармоники формы (гладкий органичный контур)
  ...
  const harm = [
    ...
    { k: 8, amp: baseRough, ph: rs() * 6.283 }, // «рваность» краёв
  ];
  ...
  // Технические данные для топо/ботаники
...
// ---------- Геометрия контура ----------
function contourPath(a, scale, cx, cy, R) {
  const n = 140; let d = '';
  for (let i = 0; i <= n; i++) {
...
function scales(layers) {
  const out = [];
  for (let k = 0; k < layers; k++) out.push(1 - (k / layers) * 0.66);
  return out;
}
function crackPath(a, cx, cy, R) {
...
// ---------- Рендеры стилей ----------
// Каждый возвращает inner-SVG (в системе координат 0 0 1000 1000).

function renderHeraldry(c, id) {
...
function renderBotanical(c, id) {
  ...
  // штриховка внутри внешнего контура
  let hatch = '';
...
function renderTopo(c, id) {
  const a = c.anatomy, cx = 500, cy = 480, R = 210;
  // изолинии = слои, но с равномерным шагом для «карты»
...
// звезда
function starPath(cx, cy, r, fill, stroke) {
  let d = '';
  for (let i = 0; i < 10; i++) {
...
// лента с девизом
function ribbon(text, id, fill, stroke, textColor) {
  return `<g transform="translate(0 838)">
    ...
    <text x="500" y="37" font-family="'Playfair Display',serif" font-size="26" fill="${textColor}" text-anchor="middle" letter-spacing="1">«${text}»</text>
  </g>`;
}

export function renderArt(c, styleKey, idPrefix) {
...
export function fullArtSVG(c, styleKey, idPrefix) {
...
// ---------- Детальная рамка-багет по редкости ----------
// stops: [блик, металл, тень, провал]; gem: [блик, глубина]
const FRAME_TIERS = {
...
const W_ = 1000;
function ringPath(o, w) { return `M${o} ${o} H${W_ - o} V${W_ - o} H${o} Z M${o + w} ${o + w} V${W_ - o - w} H${W_ - o - w} V${o + w} Z`; }
function perimItems(inset, step) {
  const lo = inset, hi = W_ - inset, len = hi - lo, n = Math.max(3, Math.round(len / step)), s = len / n, out = [];
  for (let i = 0; i < n; i++) { const t = lo + s * (i + 0.5); out.push([t, lo, 0], [hi, t, 90], [t, hi, 180], [lo, t, 270]); }
  return out;
}
function beadCourse(inset, r, step, col, edge) { return perimItems(inset, step).map(([x, y]) => `<circle cx="${x.toFixed(1)}" cy="${y.toFixed(1)}" r="${r}" fill="${col}" stroke="${edge}" stroke-width="0.6"/>`).join(''); }
function dentilCourse(inset, col, edge) { return perimItems(inset, 24).map(([x, y, a]) => `<g transform="translate(${x.toFixed(1)} ${y.toFixed(1)}) rotate(${a})"><rect x="-5" y="-6.5" width="10" height="13" fill="${col}" stroke="${edge}" stroke-width="0.6"/></g>`).join(''); }
function ropeCourse(inset, hi, lo) { return perimItems(inset, 13).map(([x, y, a]) => `<g transform="translate(${x.toFixed(1)} ${y.toFixed(1)}) rotate(${a})"><path d="M-7 5 Q0 -6 7 5" fill="none" stroke="${hi}" stroke-width="2.6" stroke-linecap="round"/><path d="M-7 8 Q0 -2 7 8" fill="none" stroke="${lo}" stroke-width="1.6" stroke-linecap="round" stroke-opacity="0.55"/></g>`).join(''); }
function gemShape(cx, cy, r, id, T) {
  let prongs = '';
...
function cornerArt(rarityKey, T) {
  const c = 58, scroll = rarityKey === 'heroic' || rarityKey === 'legendary' || rarityKey === 'mythic';
  const pts = [[c, c, 0], [W_ - c, c, 90], [W_ - c, W_ - c, 180], [c, W_ - c, 270]];
...
export function renderFrame(rarityKey, id) {
...
export function fullFrameSVG(rarityKey, id) {
...
// ---------- Монограмма «М» (герб дома) ----------
export function renderMonogram(id) {
  const ip = id || 'mono';
...
// ---------- Печать-сертификат (редкость зависит от cert) ----------
export function renderSeal(id, cert) {
  const ip = id || 'seal';
  const ct = cert || { title: 'ПОДЛИННИК', hi: '#fbeec0', mid: '#dcb74e', lo: '#9a7620', ink: '#3a2c10', idx: 1 };
  let ticks = '';
...
// ---------- Локатор: показывает, на какой части тела мозоль ----------
const LOCATOR_ICON = {
...
export function renderLocator(c, id) {
...
export function fullLocatorSVG(c, id) {
  return renderLocator(c, id);
}

// ---------- Экспортная карточка (для PNG) ----------
export function buildExportSVG(c, styleKey, signature) {
  const W = 1080, H = 1350;
  const r = c.rarity;
